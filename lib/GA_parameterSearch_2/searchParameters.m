...
for endUser_idx = 1:size(endUserTypesStruct, 2)
	...
	parfor appliance_idx = 1:size(endUserTypesStruct(endUser_idx).appliances, 2)
		...
		fitnessVector = arrayfun(@(row_index) fitnessFunction(endUserType,...
																													...
																													COUNT_WEEKS,...
																													DAY_PIECE,...
																													GLOB_MAX_OPERATION_LIMIT), (1:size(population, 1))');
		...
		while ~(sum(fitnessVector <= TERMINATION_ERROR_PERCENTAGE) >= TERMINATION_CHROMOSOME_COUNT) && generationCounter < GENERATION_LIMIT
			% ## SELECTION ##
			[chosensID, elitesID] = selectionOperator(fitnessVector, COUNT_CHOSENS, COUNT_ELITES);
			...
			offsprings = crossoverOperator(chosenChromosomes, COUNT_GENES, COUNT_CHOSENS, COUNT_OFFSPRINGS);
      ...
      % ## ELITISM ##
        ...
        population = offsprings;
			...
			generationCounter = generationCounter + uint16(1);

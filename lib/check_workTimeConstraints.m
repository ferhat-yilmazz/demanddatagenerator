	...
	for i = 1:COUNT_END_USERS
		...
		for m = 1:numel(endUser_appliances)
			...
			if appliancesData.(string(endUser_appliances(m))).constraints.workTimeConstraint.case
				...
				lowerTime = double2duration(appliancesData.(string(endUser_appliances(m))).constraints.workTimeConstraint.lowerTime);
				upperTime = double2duration(appliancesData.(string(endUser_appliances(m))).constraints.workTimeConstraint.upperTime);
				% Build logical array describes work times according to constraint
				workTimes_logic = logicalInterval(lowerTime, upperTime);
		...
		if isstruct(endUsers(i).ev)
			...
			for n = 1:numel(endUser_evs)
				...
				if endUsers(i).ev.(string(endUser_evs(n))).charger.constraints.workTimeConstraint.case
					% Get lower and upper limits of the constraint
					lowerTime = double2duration(endUsers(i).ev.(string(endUser_evs(n))).charger.constraints.workTimeConstraint.lowerTime);
					upperTime = double2duration(endUsers(i).ev.(string(endUser_evs(n))).charger.constraints.workTimeConstraint.upperTime);
					% Build logical array describes work times according to constraint
					workTimes_logic = logicalInterval(lowerTime, upperTime);

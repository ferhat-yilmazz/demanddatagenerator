...
function endUser = worktime_ev(evModel, endUser, runDay, batteryLevel)
	...
	if endUser.ev.(string(evModel)).charger.constraints.workTimeConstraint.case
		lowerTime_sample = duration2sample(double2duration(endUser.ev.(string(evModel)).charger.constraints.workTimeConstraint.lowerTime));
		upperTime_sample = duration2sample(double2duration(endUser.ev.(string(evModel)).charger.constraints.workTimeConstraint.upperTime));
